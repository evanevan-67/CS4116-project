...
$query = "SELECT connectionid, CASE WHEN user1id = $userid THEN 'user1id'
                                    ...
                                    FROM connections WHERE (user1connection = $userid OR user1connection = $profileid) 
                                                    AND (user2connection = $userid OR user2connection = $profileid);";
...
if(mysqli_num_rows($result) == 1) {
    ...
    if ($row['matched_column'] == user1id){
        $stmt2 = $conn->prepare("UPDATE connections SET user1connection = $connection WHERE connectionid = $row['connectionid'];");
        if ($stmt2->execute()) {
        ...
        $stmt2 = $conn->prepare("UPDATE connections SET user2connection = $connection WHERE connectionid = $row['connectionid'];");
        if ($stmt2->execute()) {
            echo "Connection updated successfully";
        } else {
            echo "Error: " . $stmt2->error;
...
if ($row2['user1connection'] && $row2['user2connection']){
    $stmt3 = $conn->prepare("UPDATE connections SET connectionstatus = 1 WHERE connectionid = $row['connectionid'];");
        if ($stmt3->execute()) {
    ...
    $stmt5 = $conn->prepare("UPDATE connections SET connectionstatus = 0 WHERE connectionid = $row['connectionid'];");
        if ($stmt5->execute()) {
